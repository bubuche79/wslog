...
function add_tab(tab, active)
	...
	http.write(string.format("<a href='/cgi-bin/wsview/observations/%s'>%s</a>", tab, i18n_fmt("tab_" .. tab)))
	http.write([[</li>]])
end
...
function obs_table(year, month, day)
end
...
<form name="test" action="/cgi-bin/wsview/climate/">]])
	...
	if (view == "table") then
		obs_table(year, month)
	else
		obs_charts(year, month)
	end
