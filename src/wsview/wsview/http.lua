...
local function modload(s)
	local hasmod, mod, func, i, module
	...
	for k, v in pairs(s) do
		...
		if hasmod and mod[v] then
			func = mod[v]
			break
		else
			module = module .. "." .. v
			hasmod, mod = pcall(require, module)
		end
	end

	if not func and hasmod and mod["index"] then
		func = mod["index"]
	end
...
function http.dispatch(env)
	...
	local func, i = modload(s)

	if func then
		env.ARGS = subrange(s, i)
		func(env, params)
	else
		http.status(400)
	end
